...
`\l__atmz_rand_int`に対応した向きの2マス先の状態を取得し、壁でないなら（道ならば）`\l__atmz_rand_int`をインクリメントします。この処理にすることで、`\l__atmz_rand_int`が移動できない場合は次の向きに任せることができるからです。また、`\l__atmz_rand_int`の最大値は<JwAki />`\l__atmz_count_tblr_int`となっているため、4までにかならず掘り進めることができます。
...
これを作るにあたって<JwAki />[@zr_tex8rさん](https://twitter.com/zr_tex8r)と<JwAki />[@wtsnjpさん](https://twitter.com/wtsnjp/)からTwitterでたくさんの助言を頂きました。ありがとうございました。
