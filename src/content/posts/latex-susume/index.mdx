...
publishedDate: 2021-02-14T00:00:00+09:00
...
TeX（<TeX />/テック/テフ）は、スタンフォード大学のDonald E. Knuth教授が自ら開発した**フリーの組版システム**です。TeXはMS Wordなどの見たままを出力するWYSIWYGソフトウェアと異なり、執筆・組版・出力の各段階を分けて処理を行います。**執筆に集中でき**、さまざまなプラットフォーム上で**高品質な文書を出力する**ことができます。
...
テキストエディタを用いて下に示すソースファイルを作成します。ソースコードの具体的な説明は[TeXソースの基本構成](#texソースの基本構成)を参照してください。
...
改ページを行なうコマンドには、`\pagebreak`、`\newpage`、`\clearpage`などがあります。コマンドごとに細かな仕様が異なりますが基本的に改ページに用いられます。
...
今回は、upLaTeXを中心にLaTeXの**ごく一部**を紹介しました。本記事では以下に示すような紹介できてないです。
