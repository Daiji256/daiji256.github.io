...
publishedDate: 2021-11-12T00:00:00+09:00
...
- [`tex2svg`](http://tex2svg.herokuapp.com/)：MathJaxによって数式を生成します。HerokuでAPIが公開されていて、レスポンスは少し遅いです。
...
単なる画像としてのSVGなので、数式をコピペできません。SVG内に見えないテキストを用意してコピペできるようにしたいと思います。`replace`を使って<JwAki />`</svg>`<JwAki />を<JwAki />`<text> ... </text></svg>`<JwAki />に置換して、無理やりテキスト情報を入れます。CSSテキストのスタイルを指定します。色を無色`rgba(0,0,0,0)`<JwAki />にします。SVG内のフォントサイズはちょっと**アレ**ですごく小さいみたいなので、適当に超大きくなるように設定します。
