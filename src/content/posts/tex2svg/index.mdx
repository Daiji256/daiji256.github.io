...
tex2svgは[`https://tex2svg.herokuapp.com/?`+`q=E=m^2`+`&inline=true`](https://tex2svg.herokuapp.com/?&q=E=m^2&inline=true)のように叩きます。`q=`<JwAki />に表示したい数式のLaTeXコードを書きます。`&inline=true`を書くとインライン表示になります。また、ディスプレイ表示のとき`class=tex`、インライン表示のとき`class=i-tex`にしてくれます。
...
MathJaxにはパッケージを追加したり、フォントを変更する機能があります。tex2svgではデフォルト状態になっています。MathJaxの設定をやりたい場合は自分でAPIを用意してください。ソースコードは[tex2svg](https://github.com/atishay/tex2svg/)を参考にすれば良いと思います。ローカルでHugoを使う場合は`localhost:3000`にAPIをたてたとしたら、`https://tex2svg.herokuapp.com/`<JwAki />を`http://localhost:3030/`<JwAki />に置きかえらば良いです。僕はGitHub Actionsで`hugo`しているので、tex2svgを直接利用しています[^myapi]。
